 ...
 *   4. normalises + validates the lead, then delivers it to the CRM
 *      (LeadOptimizer/GoHighLevel) via `api/_lib/leadoptimizer.ts`, falling
 *      back to the generic `LEAD_ENDPOINT` forward (e.g. Formspree) if the
 *      CRM isn't configured or the delivery fails.
 *
 * INERT BY DEFAULT (parity with api/reviews.ts): until either the CRM or
 * `LEAD_ENDPOINT` is set this returns 501 and the client keeps using its
 * existing direct path, so the form never breaks. Turnstile is only
 * *enforced* once `TURNSTILE_SECRET_KEY` is set — so you can roll it out in
 * stages.
 *
 * SERVER-ONLY env (Vercel → Settings → Environment Variables, NO VITE_ prefix):
 *   LEADOPTIMIZER_WEBHOOK_URL              — CRM inbound webhook (preferred)
 *   LEADOPTIMIZER_PIT_TOKEN + LOCATION_ID  — CRM API credentials (alternative)
 *   LEAD_ENDPOINT                          — fallback forward target (e.g. Formspree)
 *   TURNSTILE_SECRET_KEY                   — Cloudflare Turnstile secret (enables verification)
 */

import { validateLead } from './_lib/leadModel.js';
import { deliverToLeadOptimizer, leadOptimizerConfigured } from './_lib/leadoptimizer.js';
...
export default async function handler(request: Request): Promise<Response> {
  ...
  // Server-only delivery targets. Neither set → tell the client to use its fallback.
  const leadEndpoint = process.env.LEAD_ENDPOINT;
  const crmConfigured = leadOptimizerConfigured();
  if (!leadEndpoint && !crmConfigured) return json({ error: 'Lead proxy not configured' }, 501);
  ...
  // Deliver server-side. CRM first (never reaches the public bundle); fall
  // back to the generic forward target if the CRM isn't configured or fails.
  if (crmConfigured) {
    const { ok, lead, errors } = validateLead(payload);
    if (!ok) return json({ error: 'Invalid lead', fields: errors }, 400);
    const result = await deliverToLeadOptimizer(lead!);
    if (result.ok) return json({ ok: true }, 200);
    // CRM delivery failed — fall through to the generic endpoint if we have one.
    if (!leadEndpoint) return json({ error: 'Delivery failed' }, 502);
  }
