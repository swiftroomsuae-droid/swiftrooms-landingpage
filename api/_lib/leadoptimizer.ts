...
import type { NormalizedLead } from './leadModel.js';
import { splitName } from './leadModel.js';
...
/** True when the lead's attribution points at a Google Ads click/campaign. */
export function isGoogleAdsLead(lead: NormalizedLead): boolean {
  const a = lead.attribution;
  if (a.gclid) return true;
  const source = a.utmSource?.toLowerCase() || '';
  const medium = a.utmMedium?.toLowerCase() || '';
  return source.includes('google') && (medium.includes('cpc') || medium.includes('ppc') || medium.includes('paid') || medium.includes('ad'));
}

/** Tags that drive LeadOptimizer pipeline routing + workflow automation. */
export function tagsFor(lead: NormalizedLead): string[] {
  const tags = new Set<string>(['LANDING PAGE LEAD', `type:${lead.leadType}`]);
  ...
  if (isGoogleAdsLead(lead)) tags.add('Google Ads Landing Page Leads');
...
/**
 * Field names below match the "Website Leads — Swiftrooms (Inbound Webhook)"
 * GHL workflow's Create Contact mapping exactly. GHL's UI displays these as
 * friendly Title Case labels ("Form Type", "Page Source", ...) but the
 * underlying merge keys it reads from the webhook body are lowerCamelCase —
 * confirmed empirically by probing the live webhook with parallel key
 * variants (see PR discussion). Keep these two in sync if either changes.
 * Extra keys are harmless; GHL ignores anything it doesn't map.
 */
function webhookBodyFor(lead: NormalizedLead) {
  const { firstName, lastName } = splitName(lead.fullName);
  const a = lead.attribution;
  return {
    firstName,
    ...
    pageSource: a.landingPage || lead.sourceSection || 'swiftrooms-landing-page',
    contactMethod: lead.preferredContactMethod,
    projectType: lead.projectType,
    location: lead.siteLocation,
    projectStage: lead.projectType,
    budgetScope: lead.budgetRange,
    propertyType: lead.propertyType,
    productsNeeded: lead.productInterest.join(', '),
    timeline: lead.timeline,
    visitType: lead.leadType === 'showroom' ? 'Showroom Visit' : '',
    preferredTime: lead.showroomTime,
    visitorRole: '',
    resourceTitle: '',
    marketingConsent: lead.marketingConsent ? 'Yes' : 'No',
    // Contact source is a confirmed-working field (unlike the custom fields
    // above, it doesn't depend on the trigger's learned schema). This CRM
    // also receives leads from the main website via a separate integration,
    // so every lead delivered through THIS landing page carries a fixed,
    // literal label distinguishing it from those — with the Google Ads tag
    // (below) layered on top for campaign-level filtering.
    source: 'LANDING PAGE LEAD',
    tags: tagsFor(lead),
    isGoogleAdsLead: isGoogleAdsLead(lead),
...
async function deliverViaWebhook(lead: NormalizedLead): Promise<DeliveryResult> {
  ...
  const body = webhookBodyFor(lead);
  let res: Response;
  try {
