...
import { trackLeadFormSubmit, trackThankYouPageView, trackWhatsAppClick } from '../utils/analytics';
...
export function LeadForm({ autoOpen = false, ctaVariant = 'green', listenForOpenEvent = false }: { autoOpen?: boolean; ctaVariant?: 'green' | 'white'; listenForOpenEvent?: boolean }) {
  ...
  // Captured once per page load: UTM/ad-click params identify Google Ads
  // traffic so the CRM can tag and route these leads distinctly. Read from
  // the URL at mount time, before any client-side navigation could drop them.
  ...
  // Fires once per visit to the thank-you screen (the in-form equivalent of a
  // thank-you page view) so GTM/GA4 can record it as a conversion event.
  useEffect(() => {
    if (currentStep >= totalSteps) trackThankYouPageView();
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Always submit through the same-origin /api/lead proxy: it forwards
      // to the CRM server-side (no CRM URL ever reaches the public bundle).
      // Turnstile verification only runs if a site key is configured.
      ...
      if (res.status === 501) {
        // Proxy present but no CRM/lead endpoint configured server-side —
        // don't lose the lead; fall back to the direct endpoint.
        await deliverDirect();
      } else if (!res.ok) {
          ...
          >
            ...
            <div className="bg-white border border-[#25D366]/30 rounded-[4px] p-4 lg:p-5 max-w-sm mx-auto shadow-[0_16px_40px_#0079691f] hover:shadow-xl transition-shadow">
              ...
              <a
                ...
                onClick={() => trackWhatsAppClick('971505269149')}
